...
(function($) {
  $.fn.dependsOn = function(parent) {       
    ...
    caller.bind('click iterate', function(event){
      ...
      parent.attr('checked', true).button('refresh').trigger('iterate', ['Iterate', 'Event']);
    });              
   ...
   $.fn.dependsOn.check = function(parent, child, opts){
     if(!parent.attr('checked') || !$(opts.origparent).attr('checked'))
          child.attr('checked', false).button('refresh').change(); //uncheck the checked child, and trigger the change event so that any potential grandchildren also gets updated
   };
})(jQuery);